...
class Learning(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    """Learning entity."""
    ...
    evidence_summary: Mapped[str | None] = mapped_column(nullable=True)
    ...
    # Constraints
    __table_args__ = (
        Index("ix_learnings_company_id", "company_id"),
        Index("ix_learnings_objective_id", "objective_id"),
        Index("ix_learnings_company_id_status", "company_id", "status"),
        Index("ix_learnings_evidence_id", "evidence_id"),
    )

    def __repr__(self) -> str:
        return f"<Learning id={self.id} company_id={self.company_id}>"
